			...
			if sys.platform.startswith('win') or sys.platform.startswith('darwin'):
				...
				if len(faces) > 0:
					for (x, y, w, h) in faces:
						...
						if confidence > 100:
							cv2.putText(img, str("unknow"), (x + 5, y + h + 30), cv2.FONT_HERSHEY_DUPLEX, 1, (0, 255, 0), 2)
						else:
							cv2.putText(img, self.name[int(id)][1], (x + 5, y + h + 30), cv2.FONT_HERSHEY_DUPLEX, 1, (0, 255, 0), 2)
